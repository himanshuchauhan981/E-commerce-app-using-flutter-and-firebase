...
class ShoppingBagService {
  ...
  Future<String> add(String productId, String size, String color, int quantity) async {
    ...
    if (uid != null) {
      ...
      if (bagItems.docs.length == 0) {
        await _shoppingBagReference.add({
          'userId': uid,
          'products': [
        ...
        msg = await update(productId, size, color, quantity, bagItems);
      }
    } else {
      msg = 'Please login to add product to shopping bag';
    }
    return msg;
  }
  ...
  Future<List> list() async {
    ...
    if (totalBags != 0) {
      ...
      for (int i = 0; i < bagItemDetails.length; i++) {
        ...
        String image = productRef['imageId'][0];
  ...
  Future<void> remove(String id) async {
    String? uid = await userService.getUserId();
    if (uid != null) {
      await _shoppingBagReference.where('userId', isEqualTo: uid).get().then(
        (value) {
          value.docs.forEach(
            (docRef) async {
